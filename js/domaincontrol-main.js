...
(function () {
	...
	const DomainControl = {
		...
		showDomainDetail: function (id) {
			...
			if (hosting) {
				hostingEl.innerHTML = `<a href="#" onclick="DomainControl.switchTab('hostings'); setTimeout(() => DomainControl.showHostingDetail(${hosting.id}), 100); return false;" style="color: var(--color-primary-element-element); text-decoration: none;">${this.escapeHtml(hosting.provider)}${hosting.plan ? ' - ' + this.escapeHtml(hosting.plan) : ''}</a>`;
			} else {
		...
		showClientDetail: function (id) {
			...
			if (emailEl) {
				if (client.email) {
					emailEl.innerHTML = `<a href="mailto:${this.escapeHtml(client.email)}" style="color: var(--color-primary-element-element); text-decoration: none;">${this.escapeHtml(client.email)}</a>`;
				} else {
					emailEl.textContent = '-';
				}
			}
			...
			if (phoneEl) {
				if (client.phone) {
					phoneEl.innerHTML = `<a href="tel:${this.escapeHtml(client.phone)}" style="color: var(--color-primary-element-element); text-decoration: none;">${this.escapeHtml(client.phone)}</a>`;
				} else {
					phoneEl.textContent = '-';
				}
			}
		...
		showWebsiteDetail: function (id) {
			...
			if (domain) {
				domainInfoEl.innerHTML = `<div class="mini-item">
					<a href="#" onclick="DomainControl.switchTab('domains'); setTimeout(() => DomainControl.showDomainDetail(${domain.id}), 100); return false;" style="color: var(--color-primary-element-element); text-decoration: none; font-weight: 500;">${this.escapeHtml(domain.domainName)}</a>
			...
			if (hosting) {
				hostingInfoEl.innerHTML = `<div class="mini-item">
					<a href="#" onclick="DomainControl.switchTab('hostings'); setTimeout(() => DomainControl.showHostingDetail(${hosting.id}), 100); return false;" style="color: var(--color-primary-element-element); text-decoration: none; font-weight: 500;">${this.escapeHtml(hosting.provider)}${hosting.plan ? ' - ' + this.escapeHtml(hosting.plan) : ''}</a>
					<div style="font-size: 12px; color: var(--color-text-maxcontrast); margin-top: 4px;">
		...
		showInvoiceDetail: function (id) {
			...
			if (progressEl) {
				const progressColor = paymentPercent === 100 ? 'var(--color-element-success)' :
					(paymentPercent >= 50 ? 'var(--color-element-warning)' : 'var(--color-primary-element-element)');
				...
				<div class="list-item project-item" data-id="${proj.id}" style="position: relative;">
					...
					<div class="project-progress-mini" style="flex-shrink: 0; margin: 0 40px; text-align: right;">
						...
						<div class="progress-bar" style="width: 120px; height: 8px; background: var(--color-background-dark); border-radius: 4px; overflow: hidden;">
							<div style="width: ${progress}%; height: 100%; background: ${progress === 100 ? 'var(--color-element-success)' : 'var(--color-primary-element-element)'}; transition: width 0.3s;"></div>
						</div>
					</div>
				...
				<div class="list-item task-item ${overdue ? 'status-critical' : ''} ${task.status === 'cancelled' ? 'task--cancelled' : ''}" data-id="${task.id}" style="position: relative; overflow: hidden;">
					...
					<div class="list-item__content">
						...
						<div>
								...
								<div class="list-item__meta">
									<span style="color: var(--color-primary-element-element); font-weight: 500;">${project ? project.name : 'Genel'}</span>
