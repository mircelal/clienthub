...
class TaskController extends Controller
{
	...
	public function create(): JSONResponse
	{
		try {
			...
			if ($task->getDueDate()) {
				...
				$eventUid = $this->calendarService->addTaskEvent(
					$this->userId,
					$task->getTitle(),
					$task->getDueDate(),
					$description
				...
				// Store event UID in notes
				if ($eventUid) {
					$currentNotes = $task->getNotes() ?: '';
	...
	public function update(int $id): JSONResponse
	{
		try {
			...
			$oldNotes = $task->getNotes() ?: '';
			
			// Extract existing calendar UID from notes
			$existingUid = null;
			...
			if (isset($data['dueDate'])) {
				...
				if ($task->getDueDate()) {
					...
					if ($existingUid) {
						$this->calendarService->removeTaskEvent($this->userId, $existingUid);
						// Remove UID from notes
					...
					// Store new UID in notes
					if ($eventUid) {
						$task->setNotes(trim($oldNotes) . "\n[CALENDAR_UID:" . $eventUid . "]");
					} else {
						$task->setNotes($oldNotes);
					}
				} elseif ($existingUid) {
					// Remove event if due date is cleared
					$this->calendarService->removeTaskEvent($this->userId, $existingUid);
					$task->setNotes(preg_replace('/\s*\[CALENDAR_UID:[^\]]+\]\s*/', '', $oldNotes));
				}
			} elseif (isset($data['title']) && $existingUid && $task->getDueDate()) {
				// Update event if title changed and due date exists
				$this->calendarService->removeTaskEvent($this->userId, $existingUid);
				$description = $task->getDescription() ?: '';
				$eventUid = $this->calendarService->addTaskEvent(
					$this->userId,
	...
	public function delete(int $id): JSONResponse
	{
		try {
			...
			if (preg_match('/\[CALENDAR_UID:([^\]]+)\]/', $notes, $matches)) {
				$this->calendarService->removeTaskEvent($this->userId, $matches[1]);
			}
