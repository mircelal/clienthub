...
class InvoiceController extends Controller {
	...
	public function create(): JSONResponse {
		try {
			...
			$invoice->setInvoiceNumber($data['invoiceNumber'] ?? $this->mapper->generateInvoiceNumber($this->userId));
	...
	public function update(int $id): JSONResponse {
		try {
			...
			if (isset($data['clientId'])) $invoice->setClientId((int)$data['clientId']);
			if (isset($data['invoiceNumber'])) $invoice->setInvoiceNumber($data['invoiceNumber']);
			if (isset($data['issueDate'])) $invoice->setIssueDate($data['issueDate']);
			if (isset($data['dueDate'])) $invoice->setDueDate($data['dueDate']);
			if (isset($data['totalAmount'])) $invoice->setTotalAmount((float)$data['totalAmount']);
			if (isset($data['currency'])) $invoice->setCurrency($data['currency']);
