...
export default {
	...
	methods: {
		...
		async loadPayments() {
			// Payments are now transactions (income type)
			// This method is kept for backward compatibility but does nothing
			// All payment data comes from transactions
		},
		async loadProjects() {
			try {
		...
		calculateReports() {
			...
			// Calculate total income from transactions (income type)
		...
		calculateTopClients() {
			const clientIncome = {}
			// Use income transactions (payments are now transactions)
			const incomeTransactions = this.transactions.filter(t => t.type === 'income')
			incomeTransactions.forEach(transaction => {
				const clientId = transaction.clientId || transaction.client_id
				if (!clientId) return
				if (!clientIncome[clientId]) {
					clientIncome[clientId] = 0
				}
				clientIncome[clientId] += parseFloat(transaction.amount) || 0
			})
