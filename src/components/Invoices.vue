                ...
                <div class="financial-summary">
                    ...
                    <div class="summary-row summary-total">
                        ...
                        <span class="summary-value font-mono" :class="getBalanceClass(selectedInvoice)">
                            {{ formatCurrency(selectedInvoice.balance !== undefined && selectedInvoice.balance !== null ? selectedInvoice.balance : ((selectedInvoice.total || selectedInvoice.totalAmount || 0) - (selectedInvoice.paidAmount || 0))) }}
                        </span>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        async loadData() {
            this.loading = true
            try {
                // Load invoices, clients, and transactions
        ...
        calculateStats() {
            ...
            // Calculate total revenue from income transactions (actual payments)
            const incomeTransactions = this.transactions.filter(t => t.type === 'income')
            this.stats.totalRevenue = incomeTransactions.reduce((acc, curr) => {
        ...
        getBalanceClass(invoice) {
            if (!invoice) return ''
            const balance = invoice.balance !== undefined && invoice.balance !== null ? invoice.balance : ((invoice.total || invoice.totalAmount || 0) - (invoice.paidAmount || 0))
        ...
        async handleItemSaved() {
            if (this.selectedInvoice && this.selectedInvoice.id) {
                const selectedId = this.selectedInvoice.id
                await this.loadInvoiceDetails(selectedId)
                // Reload invoice to get updated total
                await this.loadData()
                
                // Update selectedInvoice reference to get updated totals from list
                const updated = this.invoices.find(inv => inv.id === selectedId)
                if (updated) {
        ...
        async handlePaymentSaved() {
            if (this.selectedInvoice && this.selectedInvoice.id) {
                ...
                // Update selectedInvoice reference to get updated paidAmount/balance from list
                const updated = this.invoices.find(inv => inv.id === selectedId)
                if (updated) {
                    this.selectedInvoice = updated
                }
