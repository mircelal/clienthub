<template>
	<div class="app-content-wrapper">
		<!-- LEFT PANE: LIST -->
		<div class="app-content-list" :class="{ 'mobile-hidden': isMobile && selectedProject }">
			<ProjectList
		...
		<div class="app-content-details" :class="{ 'mobile-hidden': isMobile && !selectedProject }">
			<!-- Empty State -->
			<div v-if="!selectedProject" class="empty-content">
				<div class="empty-content-icon"><Folder :size="64" /></div>
				<h2 class="empty-content-title">{{ translate('domaincontrol', 'Select a project') }}</h2>
			</div>
...
export default {
	...
	mounted() {
		...
		window.addEventListener('select-project', this.handleSelectProjectEvent)
	},
	beforeUnmount() {
		window.removeEventListener('resize', this.handleResize)
		document.removeEventListener('click', this.handleClickOutside)
		window.removeEventListener('select-project', this.handleSelectProjectEvent)
	...
	methods: {
		...
		async loadTabData(tab, projectId) {
			...
			switch (tab) {
				case 'tasks-time':
					...
					break
				case 'documents':
					// Files and Notes load their own data via watch
					break
				case 'financials':
		...
		handleSelectProjectEvent(event) {
...
.user-select-item--selected {
	background-color: var(--color-primary-element-element-light);
