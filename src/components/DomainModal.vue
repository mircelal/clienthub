...
export default {
    ...
    methods: {
        ...
        loadDomainData() {
            ...
            this.formData = {
                id: d.id || null,
                clientId: d.clientId || d.client_id || this.presetClientId || '',
                domainName: d.domainName || d.domain_name || '',
                registrar: d.registrar || '',
                registrationDate: (d.registrationDate || d.registration_date) ? (d.registrationDate || d.registration_date).split(' ')[0] : '',
                expirationDate: (d.expirationDate || d.expiration_date) ? (d.expirationDate || d.expiration_date).split(' ')[0] : '',
                price: d.price || '',
                currency: d.currency || 'USD',
                renewalInterval: d.renewalInterval || d.renewal_interval || '1',
                panelNotes: d.panelNotes || d.panel_notes || '',
                notes: d.notes || '',
            }
        },
        closeModal() {
        ...
        async saveDomain() {
            const domainName = (this.formData.domainName || '').trim()
            const clientId = this.formData.clientId || this.presetClientId
            ...
            try {
                const data = {
                    ...
                    registrar: (this.formData.registrar || '').trim(),
                    registrationDate: this.formData.registrationDate || '',
                    expirationDate: this.formData.expirationDate || '',
                    price: this.formData.price ? parseFloat(this.formData.price) : 0,
                    currency: this.formData.currency || 'USD',
                    renewalInterval: this.formData.renewalInterval || '1',
                ...
                const errorMessage = error.response?.data?.error || error.message || this.translate('domaincontrol', 'Error saving domain')
                alert(errorMessage)
            } finally {
                this.saving = false
            }
